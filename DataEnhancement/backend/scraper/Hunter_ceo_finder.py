...
class CompanyInfoRetriever:
    def __init__(self, hunter_api_key=None):
        ...
        if not self.hunter_api_key:
            raise ValueError(
                "Hunter.io API key is required. Set it as HUNTER_API_KEY environment variable or pass it directly."
            )

    def get_company_domain(self, company_name):
        """Use Hunter.io to find a company's domain name."""
        url = "https://api.hunter.io/v2/domain-search"
        params = {"company": company_name, "api_key": self.hunter_api_key, "limit": 1}
    ...
    def extract_ceo_info(self, emails):
        ...
        for email in emails:
            ...
            if position and (
                "ceo" in position
                or "chief executive officer" in position
                or ("founder" in position and "chief" in position)
            ):
                return {
        ...
        for email in emails:
            ...
            position = (email.get("position") or "").lower()  # Handle None positions safely
            if position and (
                "founder" in position or "owner" in position or "president" in position
            ):
                return {
                    "name": f"{email.get('first_name', '')} {email.get('last_name', '')}".strip(),
                    "email": email.get("value", ""),
...
def read_companies_from_csv(csv_file_path, company_name_column="company_name"):
    ...
    try:
        ...
        if company_name_column not in df.columns:
            print(
                f"Error: Column '{company_name_column}' not found in CSV. Available columns: {df.columns.tolist()}"
            )
